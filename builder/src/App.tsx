...
export default function App() {
  ...
  if (fullPreview) return <div className="flex h-screen items-center justify-center bg-zinc-950 p-8"><div className="relative h-[min(860px,calc(100vh-64px))] w-[390px] overflow-hidden rounded-[42px] border-[8px] border-zinc-800 bg-white shadow-2xl"><Button className="absolute right-3 top-3 z-10 bg-white" size="icon" variant="outline" onClick={() => setFullPreview(false)} aria-label="Close full preview"><X className="size-4" /></Button><div className="h-full overflow-y-auto">{draft.instances.map((instance) => <ComponentHost key={instance.id} componentId={instance.componentId} values={instance.values} theme={theme} />)}</div></div></div>;
