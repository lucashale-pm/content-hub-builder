...
const definitions = [heroDefinition, feedDefinition, steamDataDefinition, verticalVideoDefinition, pageContentDefinition, imageGalleryDefinition] as Definition[];
const byId = new Map(definitions.map((definition) => [definition.id, definition]));
const renderers: Record<string, (values: Values, theme: unknown) => HTMLElement> = { hero: renderHero, feed: renderFeed, "steam-data": renderSteamData, "vertical-video": renderVerticalVideo, "page-content": renderPageContent, "image-gallery": renderImageGallery };
const themes = {
...
export default function App() {
  ...
  return <div className="grid min-h-screen grid-cols-[248px_minmax(340px,1fr)_minmax(580px,2fr)] bg-zinc-100">
    ...
    <section className="flex min-h-screen flex-col border-r border-zinc-200 bg-white"><header className="border-b px-5 py-4"><p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">{mode === "builder" ? "Page authoring" : "Isolated catalogue"}</p><h1 className="mt-1 text-lg font-bold">{title}</h1></header><div className="min-h-0 flex-1 overflow-y-auto p-5"><p className="mb-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Components</p><div className="grid gap-2">{definitions.map((definition) => <button key={definition.id} className={cn("rounded-md border p-3 text-left", selectedComponentId === definition.id ? "border-zinc-900 bg-zinc-50" : "border-zinc-200 hover:bg-zinc-50")} onClick={() => { setSelectedComponentId(definition.id); if (mode === "builder") setSelectedInstanceId(null); }}><div><span className="font-semibold">{definition.name}</span></div><span className="text-xs text-zinc-500">{definition.category}</span></button>)}</div>{mode === "builder" && <Button className="mt-3 w-full" variant="outline" onClick={() => addComponent(selectedComponentId)}><Plus className="size-4" /> Add {selectedDefinition.name}</Button>}<div className="mt-7 rounded-lg border-2 border-blue-600 bg-blue-50 p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-800">{selectedInstance ? "Selected page instance" : mode === "builder" ? "New component configuration" : "Component preview controls"}</p><p className="mt-1 text-sm font-semibold text-zinc-900">{selectedDefinition.name}</p></div>{mode === "builder" && selectedInstance && <Button variant="destructive" size="sm" onClick={() => setDraft((current) => ({ ...current, instances: current.instances.filter((instance) => instance.id !== selectedInstance.id) }))}><Trash2 className="size-4" /> Delete</Button>}</div><FieldEditor fields={selectedDefinition.fields} values={currentValues} onChange={updateValues} /></div></div></section>
