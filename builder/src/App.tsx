...
import { Component, Copy, Download, Eye, GripVertical, MonitorSmartphone, PanelLeftClose, PanelLeftOpen, Pencil, Plus, Trash2, Upload, Wrench, X } from "lucide-react";
...
export default function App() {
  ...
  const duplicateInstance = (instance: Instance) => {
    const duplicate = { id: crypto.randomUUID(), componentId: instance.componentId, values: clone(instance.values) };
    setDraft((current) => {
      const index = current.instances.findIndex((item) => item.id === instance.id);
