...
const defaultFeaturedVideos = { gamesradar: "https://www.youtube.com/watch?v=LDlHHOv6MN8", pcgamer: "https://www.youtube.com/watch?v=Q6_KYI3gnFE" };
...
export default function App() {
  ...
  const setBrand = (brand: Draft["brand"]) => {
    const replaceDefaultVideo = (current: Record<string, Values>) => {
      const featured = current["featured-article"];
      const currentUrl = typeof featured?.videoUrl === "string" ? featured.videoUrl : "";
      return !currentUrl || Object.values(defaultFeaturedVideos).includes(currentUrl) ? { ...current, "featured-article": { ...featured, videoUrl: defaultFeaturedVideos[brand] } } : current;
    };
    setDraft((current) => ({ ...current, brand, instances: current.instances.map((instance) => {
      if (instance.componentId !== "featured-article") return instance;
      const currentUrl = typeof instance.values.videoUrl === "string" ? instance.values.videoUrl : "";
      return !currentUrl || Object.values(defaultFeaturedVideos).includes(currentUrl) ? { ...instance, values: { ...instance.values, videoUrl: defaultFeaturedVideos[brand] } } : instance;
    }) }));
    setNewValues(replaceDefaultVideo);
    setViewerValues(replaceDefaultVideo);
  };
