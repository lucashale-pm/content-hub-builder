...
function FieldEditor({ fields, values, onChange }: { fields: Field[]; values: Values; onChange: (next: Values) => void }) {
  ...
  return <div className="grid gap-4">{fields.filter((field) => !field.showWhen || values[field.showWhen.field] === field.showWhen.equals).map((field) => <div key={field.id} className="grid gap-1.5">
    ...
    {field.type === "radio" ? <fieldset className="grid gap-2 rounded-md border border-zinc-200 p-3"><legend className="px-1 text-xs font-semibold">{field.label}</legend><div className="grid grid-cols-2 gap-2">{field.options?.map((option) => <label key={option} className="flex items-center gap-2 rounded-md border border-zinc-200 bg-white px-2 py-2 text-xs"><input type="radio" name={field.id} value={option} checked={values[field.id] === option} onChange={() => update([field.id], option)} />{option}</label>)}</div></fieldset> : null}
    ...
    {!["textarea", "select", "radio", "checkboxes", "toggle", "object", "collection"].includes(field.type) ? <input id={field.id} className="h-9 rounded-md border border-zinc-200 bg-white px-3 text-sm" type={field.type === "url" ? "url" : "text"} value={String(values[field.id] || "")} onChange={(event) => update([field.id], event.target.value)} /> : null}
