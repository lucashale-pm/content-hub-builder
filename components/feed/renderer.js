...
export function renderFeed(values, theme = fallbackTheme) {
  ...
  header.append(heading); if (text(values.archiveLabel)) { const archive = link("hub-feed__archive", values.archiveHref); archive.textContent = `${text(values.archiveLabel)} →`; header.append(archive); } feed.append(header);
