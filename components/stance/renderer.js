...
function flair(item, className) {
  if (!text(item.flairText)) return null;
...
export function renderStance(values, theme) {
  ...
  stances.forEach((stance) => {
    ...
    if (text(stance.avatarUrl) || text(stance.author) || text(stance.role) || text(stance.flairText)) { const profile = document.createElement("div"); profile.className = "hub-stance__profile"; if (text(stance.avatarUrl)) { const avatar = document.createElement("img"); avatar.src = text(stance.avatarUrl); avatar.alt = ""; profile.append(avatar); } const person = document.createElement("div"); const nameLine = document.createElement("div"); nameLine.className = "hub-stance__name-line"; if (text(stance.author)) { const name = document.createElement("strong"); name.textContent = text(stance.author); nameLine.append(name); } const reviewerFlair = flair(stance, "hub-stance__flair"); if (reviewerFlair) nameLine.append(reviewerFlair); if (nameLine.childNodes.length) person.append(nameLine); if (text(stance.role)) { const role = document.createElement("span"); role.textContent = text(stance.role); person.append(role); } profile.append(person); card.append(profile); }
    ...
    if (comments.length) { const commentBox = document.createElement("div"); commentBox.className = "hub-stance__comments"; const inputRow = document.createElement("form"); inputRow.className = "hub-stance__comment-form"; const input = document.createElement("input"); input.placeholder = "Share your take..."; input.setAttribute("aria-label", "Share your take"); const post = document.createElement("button"); post.type = "submit"; post.textContent = "Post"; inputRow.append(input, post); const commentList = document.createElement("div"); commentList.className = "hub-stance__comment-list"; const addComment = (comment) => { const item = document.createElement("div"); item.className = "hub-stance__comment"; const nameLine = document.createElement("div"); nameLine.className = "hub-stance__name-line"; if (text(comment.username)) { const username = document.createElement("strong"); username.textContent = text(comment.username); nameLine.append(username); } const audienceFlair = flair(comment, "hub-stance__flair"); if (audienceFlair) nameLine.append(audienceFlair); if (nameLine.childNodes.length) item.append(nameLine); if (text(comment.text)) { const copy = document.createElement("p"); copy.textContent = text(comment.text); item.append(copy); } if (text(comment.upvotes)) { const votes = document.createElement("span"); votes.textContent = `▲ ${text(comment.upvotes)}`; item.append(votes); } commentList.append(item); }; comments.forEach(addComment); inputRow.addEventListener("submit", (event) => { event.preventDefault(); if (!text(input.value)) return; addComment({ username: "@you", text: input.value, upvotes: "0 upvotes" }); input.value = ""; }); commentBox.append(inputRow, commentList); card.append(commentBox); }
    section.append(card);
  });
